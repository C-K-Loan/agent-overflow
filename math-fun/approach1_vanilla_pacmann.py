"""
Approach 1: Vanilla PINN with PACMANN
======================================
Solves the convection-diffusion equation:
    u_t + beta * u_x - eps * u_xx = f(x, t)

Uses a simple feedforward neural network trained with Adam, plus
PACMANN-style collocation point movement via gradient ascent on the
squared residual every P iterations.
...
# ---------------------------------------------------------------------------
#  PACMANN: gradient-ascent point movement
# ---------------------------------------------------------------------------

def pacmann_move(model, x_r, t_r, beta, eps, source_fn,
                 x_lo, x_hi, t_lo, t_hi,
                 step_size=0.001, num_steps=5):
    """
    Move collocation points toward regions of higher squared residual
    using gradient ascent w.r.t. the point coordinates.

    Points that leave the domain are resampled uniformly.
    """
    model.eval()  # freeze batch-norm / dropout if any (not used here, but safe)

    # Detach from any existing graph and make leaf tensors
    x_pts = x_r.detach().clone().requires_grad_(True)
    t_pts = t_r.detach().clone().requires_grad_(True)
    ...
    for _ in range(num_steps):
        ...
        # Gradient ascent (maximize squared residual)
        with torch.no_grad():
            x_pts += step_size * grad_x
            t_pts += step_size * grad_t
...
def train(example, beta, eps, save_dir,
    ...
    for epoch in range(1, epochs + 1):
        ...
        # ---- PACMANN point movement ----
        if epoch > 1 and (epoch - 1) % pacmann_period == 0:
            x_r, t_r = pacmann_move(
    ...
    label = f"Approach1_Ex{example}_beta{beta}_eps{eps}"
...
if __name__ == "__main__":
    device = "cpu"
    save_dir = Path("/Users/christianloan/Agent/math-fun/results/approach1")
