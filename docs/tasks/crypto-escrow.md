...
| # | Threat | Severity | Decision |
|---|--------|----------|----------|
| **1** | **FRONT-RUNNING / MEV** — answer visible in mempool | **CRITICAL** | **Commit-reveal scheme** for bounties >$50. Commit hash, wait 5 slots, reveal. |
| 2 | Binary search on approximation verifiers | ACCEPTED | Don't care — as long as solution is correct, method doesn't matter. Unlimited retries OK. |
| 3 | Unsolvable bounties (always-false verifier) | ACCEPTED | Rate limiting + reputation system handles this. Auto-refund after deadline. |
| 4 | Verifier upgraded after bounty (rug pull) | ACCEPTED | Answerers can see the derivation. Not worried. |
| 5 | Verifier drains vault via CPI | HIGH | Escrow NEVER passes writable vault to verifier. PDA-controlled. Must implement correctly. |
| 6 | Verifier DoS (compute) | MEDIUM | Simulation catches it. Compute cap 200K CU. |
| 7 | Race condition | MEDIUM | Solana serializes per account. Atomic check. |
| 8 | Platform wallet compromise | ACCEPTED | Whatever — just handle reasonably (encryption). |
| 9 | Fee rounding | ACCEPTED | Just handle correctly in code. Careful integer math. |
| 10 | IDL spoofing | MEDIUM | Fetch from on-chain PDA. Verified builds. |
| 11-14 | Solution visible, clock skew, reentrancy, sybil | LOW | Acceptable risks. Standard mitigations. |

### Commit-Reveal Protocol (for bounties >$50)

```
Phase 1: COMMIT  →  commit(bounty_id, SHA256(answer + nonce))
Phase 2: WAIT    →  5 slots (~2 seconds)
Phase 3: REVEAL  →  reveal(bounty_id, answer, nonce) → verify() → release
```

Optional for small bounties. Mandatory for >$50.
