  ...
  if (!apiKey) {
    return (
      <div className="max-w-lg mx-auto text-center py-16">
        ...
        <p className="text-gray-500 mb-6">Log in to manage your wallet.</p>
  ...
  if (!connected) {
    return (
      <div className="max-w-lg mx-auto text-center py-16">
        ...
        <p className="text-gray-500 mb-6">Connect a Solana wallet to view your dashboard.</p>
        <button
    ...
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="card p-5">
          <div className="text-xs text-gray-500 uppercase tracking-wider font-medium mb-2">SOL Balance</div>
          <div className="text-3xl font-bold font-mono text-[var(--foreground)]">
            {solBalance !== null ? solBalance.toFixed(4) : "\u2014"}
          </div>
          <div className="text-xs text-gray-500 mt-1">Solana native token</div>
        </div>
        <div className="card p-5 border-2 border-[var(--accent)]/20">
          <div className="text-xs text-gray-500 uppercase tracking-wider font-medium mb-2">USDC Balance</div>
          <div className="text-3xl font-bold font-mono text-[var(--accent)]">
            {usdcBalance !== null ? usdcBalance.toFixed(2) : "\u2014"}
          </div>
          <div className="text-xs text-gray-500 mt-1">Platform escrow balance</div>
        </div>
      </div>
      ...
      <div className="card p-5">
        ...
        {depositAddress ? (
          <div className="flex items-center gap-2">
            <code className="flex-1 bg-[var(--code-bg)] text-gray-300 px-4 py-2.5 rounded-lg text-sm font-mono truncate">
              {depositAddress}
            </code>
            <button
              onClick={copyAddress}
          ...
          <div className="text-sm text-gray-500">Loading deposit address...</div>
        )}
      </div>
      ...
      <div className="card p-5">
        ...
        <form onSubmit={handleWithdraw} className="space-y-3">
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1">Destination Address</label>
            <input
              type="text"
              value={withdrawDest}
          ...
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1">Amount (USDC)</label>
            <input
              type="number"
              min="0.01"
              step="0.01"
